...
class Connection:
	...
	def completeReq(self):
		a = str.encode(" " + self.file + " HTTP/1.0\r\n\r\n")
		self.sock.send(a)
...
input("press Enter to close some connections")
...
if number>3:
	input("press Enter to close connection 1 before request is complete")
	connections[0].close()

	input("press Enter to close connection 2 without reading the answer")
	connections[1].completeReq()
	connections[1].close()

	input("press Enter to read and print the answer of connection 3")
	connections[2].completeReq()
	connections[2].readAnswer()
	connections[2].printAnswer()
	connections[2].close()

input("press Enter to complete the rest of the requests and close connections")
